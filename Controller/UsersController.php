...
class UsersController extends AppController {
    ...
    public function excluir($id = null) {
        ...
        /* Busca o cookie de usuario cadastrado */
        $categoria = $this->Session->read('id_categoria');

        /* Se não é um usuário cadastrado então sai */        
        if (is_null($categoria)) {
            $this->Flash->error(__('Usuário nao autorizado'));
            $this->redirect('/Murals/index/');
            die();
        }

        /* Somente o administrador pode trabalhar nesta área */        
        if ($categoria <> 1) {
            $this->Flash->error(__('Usuário nao tem autorizacao'));
            $this->redirect('/Murals/index/');
            die();
        }
    ...
    public function index($id = NULL) {
        ...
        /* Somente o administrador pode trabalhar nesta área */
        if ($categoria <> 1) {
            $this->Flash->error(__('Usuário não tem autorização'));
            $this->redirect('/Murals/index/');
            die();
        }
