...
// echo 'Primeira saída formatada é ' . datefmt_format($fmt, strtotime($termoinicio));
// die();

$termoinicio = datefmt_format($fmt, strtotime($inicio));
$termofinal = datefmt_format($fmt, strtotime($final));
$dia_de_hoje = datefmt_format($fmt, time());
...
<p style="text-align:right">Rio de Janeiro, $dia_de_hoje.</p>
